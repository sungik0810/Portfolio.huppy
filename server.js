...
app.get("/peoplephoto/:pageNum", (req, res) => {
  const dateString = moment().format("YYYYMMDD");
  db.collection("UserImage")
    .find({ date: dateString })
    .sort({ num: -1 })
    .limit(21)
    .skip(req.params.pageNum * 21)
    .toArray((error, result) => {
...
app.get("/personphoto/:id", (req, res) => {
  // const dateString = moment().format('YYYYMMDD')
...
//유저사진 업로드
app.post("/upload/:id", upload.single("myImg"), (req, res) => {
...
//유저 사진 제거
app.delete("/content/delete/:_id", async (req, res) => {
  const post_id = req.params._id;
  const date = req.body.data_full.date;
  const file_name = req.body.data_full.filename;
  if (fs.existsSync(`./public/userimage/${date}/${file_name}`)) {
    try {
      db.collection("UserImage").deleteOne({ _id: ObjectId(post_id) });
      fs.unlinkSync(`./public/userimage/${date}/${file_name}`);
      res.status(200).json({ status: "삭제 성공" });
    } catch (error) {
      console.log(error);
    }
  } else {
    res.status(200).json({ status: "새로고침 후 다시 시도해주세요" });
  }
});
...
var profileStorage = multer.diskStorage({
  ...
  filename: function (req, file, cb) {
    const timeSort = new Date().getTime();
    cb(null, `huppy_profile_${timeSort}.png`);
  },
});
...
//어드민 페이지
app.use("/admin", require("./routes/adminPage.js"));
